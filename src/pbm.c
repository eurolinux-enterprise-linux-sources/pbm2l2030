 ...
 *
 * Aug 24, 1999  Thomas Paetzold <thomas.paetzold@fhtw-berlin.de>
 * 
 * Changed strcmp to strncmp in "make_pbm_stat()"  
 * 
 */
...
int make_pbm_stat(pbm_stat* pbm,FILE* fptr)
{
  ...
  if(!strncmp(line,"P1",2)) pbm->version=P1;
  if(!strncmp(line,"P4",2)) pbm->version=P4;

  if(pbm->version == none)
  {
    fprintf(stderr,"Lexmark2030: pbm_readheader(): unknown PBM magic :-( '%s'\n",line);
    return 0; 
  }
...
static int getbytes(FILE *fptr,int width,char* data)
{
...
/* Reads a single line into data which must be at least (pbm->width+7)/8
   bytes of storage */
int pbm_readline(pbm_stat* pbm,char* data)
{
  int tmp,tmp2;
